...
const fragment = `
  ...
  vec3 colorA = vec3(0.110, 0.059, 0.275); 
  vec3 colorB = vec3(0.110, 0.059, 0.275); 
...
const setScene = () => {
  ...
  const pointLight = new THREE.PointLight(0xFFFFFF, 17, 200); // Pure white light
  pointLight.position.set(-50, 0, 60);
  scene.add(pointLight);
  
  const hemisphereLight = new THREE.HemisphereLight(0xFFFFFF, 0xFFFFFF, 1.5); // Neutral white gradient
...
image.crossOrigin = "anonymous"; // ✅ Enables CORS
...
image.onload = () => {
    ...
    try {
        ...
        console.log("Image data successfully retrieved!");
    } catch (e) {
