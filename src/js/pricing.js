const approvedUrlsStage = [
  "/pages/template-long",
  "/pages/crm",
  "/pages/live-chat",
  "/pages/ticketing-system",
  "/pages/helpdesk",
  "/pages/customer-service",
  "/pages/ticketing-system-long",
  "/pages/customer-service-long",
  "/pages/helpdesk-long",
  "/pages/live-chat-long",
  "/pages/crm-long",
];
...
const planPricingInit = {
  0: "$10",
  1: "$60",
  2: "$360",
  3: "$900",
  4: "Contact us",
  5: "Only available for monthly subscription",
  6: "$50",
  7: "$300",
  8: "$750",
};
const planPricingInitTemplate = {
  0: "$60",
  1: "$360",
  2: "$900",
  3: "Contact us",
...
let selectedPlan = templatePagesPaths
  ? pricingPlansTemplates[0]
  : pricingPlans[7];
...
window.onload = function () {
  ...
  const setPlanPrices = (getEl) => {
    ...
    priceTabs[0].innerHTML = templatePagesPaths
      ? planPricingInitTemplate[0]
      : planPricingInit[0];
    priceTabs[1].innerHTML = templatePagesPaths
      ? planPricingInitTemplate[1]
      : planPricingInit[1];
    priceTabs[2].innerHTML = templatePagesPaths
      ? planPricingInitTemplate[2]
      : planPricingInit[2];
    priceTabs[5].innerHTML = templatePagesPaths
      ? planPricingInitTemplate[5]
      : planPricingInit[6];
    priceTabs[6].innerHTML = templatePagesPaths
      ? planPricingInitTemplate[6]
      : planPricingInit[7];
  ...
  setPlanPrices(getEl);
  const automationDescription = document.getElementsByClassName(
    " subheading-text-content__pricing checkbox-option"
  )[0];
  const voiceDescription = document.getElementsByClassName(
    " subheading-text-content__pricing checkbox-option"
  )[1];
  const smsDescription = document.getElementsByClassName(
    " subheading-text-content__pricing checkbox-option"
  )[2];
  automationDescription &&
    (automationDescription.innerHTML = "Monthly automated interactions");

  const calculateTotalPrice = (
    automationTotalPrice,
    voiceTotalPrice,
    smsTotalPrice
  ) => {
    ...
    if (
      ...
      isStringCustom(automationTotalPrice) ||
      isStringCustom(voiceTotalPrice) ||
      isStringCustom(smsTotalPrice)
    ) {
  ...
  const calculateAddOnsPrices = (
    ...
    const displayPrice =
      typeof price === "string"
        ? price
        : "+$" + formatNumberWithCommas(price) + "/mo";
    domElement.innerHTML = displayPrice;
    return toggle ? price : 0;
  };

  const showButtonDisplay = () => {
    document.querySelectorAll(".button.pricing")[0].innerHTML =
      selectedPlan.name === "enterprise" ? "Contact us" : "Start a free trial";
    document.querySelectorAll(".button.pricing")[0].href =
      selectedPlan.name === "enterprise"
        ? "https://www.gorgias.com/demo?plan_name=" +
          selectedPlan.name +
          "&period=" +
          selectedPeriod
        : "https://www.gorgias.com/signup?plan_name=" +
          selectedPlan.name +
          "&period=" +
          selectedPeriod;
  };
  ...
  const estimatePrice = () => {
    ...
    if (selectedPlan.name == "starter") {
      Array.from(getEl("wrapper-master-select__pricing")).forEach(
        (el) => (el.style["pointer-events"] = "none")
      );
      ...
      Array.from(getEl("wrapper-master-select__pricing")).forEach(
        (el) => (el.style["pointer-events"] = "auto")
      );
    ...
    automationDropdown &&
      showHideElementByVisibility(automationDropdown, automationChecked);
    ...
    // toggle dropdowns descriptions
    // automationDescription &&
    //   showHideDropdown(automationDescription, automationChecked);
    // voiceDescription && showHideDropdown(voiceDescription, voiceChecked);
    // smsDescription && showHideDropdown(smsDescription, smsChecked);

    // toggle dropdowns descriptions by visibility
    automationDescription &&
      showHideElementByVisibility(automationDescription, automationChecked);
    voiceDescription &&
      showHideElementByVisibility(voiceDescription, voiceChecked);
    smsDescription && showHideElementByVisibility(smsDescription, smsChecked);
    // calculate add-ons prices
    const automationTotalPrice = calculateAddOnsPrices(
      automationChecked,
      automationDropdownValues,
      automationDropdown,
      automationPrice,
      automationPriceEl
    );
    const voiceTotalPrice = calculateAddOnsPrices(
      voiceChecked,
      voiceDropdownValues,
      voiceDropdown,
      voicePrice,
    ...
    const smsTotalPrice = calculateAddOnsPrices(
      smsChecked,
      smsDropdownValues,
      smsDropdown,
      smsPrice,
