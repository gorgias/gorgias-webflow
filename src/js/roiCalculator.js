...
function initialize() {
    // Define elements for easy access
    const elements = {
    ...
    // Define a custom input event
    const inputEvent = new Event("input", { bubbles: true });
    // Set up debouncing for the calculateValues function
    const debounceCalculateValues = debounce(calculateValues, 300);
    // Initialize various input elements and logic
    ...
    // Set event listeners for response and resolution time inputs
    ...
    // Function to handle input for response time and resolution time
    ...
    function handleResponseTimeInput() {
        const output = Number((Number(elements.responseTimeInput.value) -
            Number(elements.responseTimeInput.value) * 0.25).toFixed(1));
    ...
    function handleResolutionTimeInput() {
        const output = Number((Number(elements.resolutionTimeInput.value) -
            Number(elements.resolutionTimeInput.value) * 0.62).toFixed(1));
